...
| **Markdown 渲染** | md-editor-v3的 MdPreview 组件 | 只做md渲染效果，不做编辑功能  |
...
| 分类       | 方案              | 说明                            |
| -------- | --------------- | ----------------------------- |
| **设计风格** | 现代简洁大气          | 参考 Medium / Obsidian / Typora |
| **设计系统** | 参考Tailwind 色彩体系 | 目前阶段采用原生css实现                 |

### 后端集成（维持现状）

| 分类 | 技术栈 | 说明 |
|------|--------|------|
| **后端框架** | FastAPI | 已完成、功能完善 |
| **数据库** | PostgreSQL | 已完成、数据持久化 |
| **API 通信** | REST HTTP | 前后端分离、零耦合 |
...
## 4️⃣ 后端集成：FastAPI（维持现状，不用 Nuxt 后端）
...
- ✅ API 通信通过 REST，零耦合
- ⏰ **可选**：如果需要 SSR/SEO，后期再升级 Nuxt 3（FastAPI 保持不变）
...
## 5️⃣ **路由方案：Vue Router（而非 Nuxt 自动路由）**
...
## 6️⃣ Icon 库：Lucide Icons（现代、精致、高度定制）
