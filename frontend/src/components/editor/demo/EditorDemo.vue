...
// useMarkdownEditor 实例
const { editorRef, onInput, ...selectionAPI } = useMarkdownEditor({
  callbacks: {
...
onUnmounted(() => {
  if (selectionAPI) {
    selectionAPI.cleanupSelectionListener()
...
const contentChars = computed(() => {
  const content = selectionAPI.state.content || ''
...
function handleApplyInlineFormat() {
  if (!selectionAPI) return
  selectionAPI.applyInlineFormat(debugState.inlineFormat)
  updateSelectionState()
  addLog(`执行 applyInlineFormat("${debugState.inlineFormat}")`, 'success')
}

function handleApplyParagraphFormat() {
  if (!selectionAPI) return
  selectionAPI.applyParagraphFormat(debugState.paragraphFormat)
  updateSelectionState()
  addLog(`执行 applyParagraphFormat("${debugState.paragraphFormat}")`, 'success')
}

function handleInsertBlock() {
  if (!selectionAPI) return
  selectionAPI.insertBlock(debugState.blockInsertType)
  updateSelectionState()
