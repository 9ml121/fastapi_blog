 ...
 *
 * @param selectionAPI - useSelection 模块实例
 */
export function useMarkdown(selectionAPI: UseSelectionReturn) {
   ...
   * @param action - 格式类型
   */
  const toggleInlineFormat = (action: InlineFormatType): void => {
   ...
   * @param action - 段落格式类型
   */
  const toggleParagraphFormat = (action: ParagraphFormatType): void => {
    // 1. 定义前缀映射表
