...
4.  **遵循“文档注释规范”**: 我为所有重要的公共函数、类和方法编写的文档字符串，都将严格遵循以下格式：
    ```python
    def function_name(param1: Type, param2: Type) -> ReturnType:
        """单行简要描述函数功能。

        如果需要，这里可以写更详细的描述，解释函数的行为、
        设计思路或使用场景。
        ...
        Raises:
            SpecificException: 描述在何种情况下会抛出此异常。
    """
    ```
...
- **任务分解粒度要求**：Parse阶段的任务按照结构化方式拆分到最小粒度，一般采用 3 层结构
- **示例**：
    ```markdown
    # 项目开发进展
...
- **关键步骤**：
    - Step1: 按照前面设计方案，向用户讲解此次编码思路，重点讲解要用到的关键技术点
    - Step2: 自主编写完整代码框架，并完成一部分样例代码， 然后执行`ruff` + `mypy` 检查代码质量和格式化(*我做你看*)
        - ⚠️ **检查点**：不要一下完成所有代码，每个新知识点在有参考样例的情况下，都应该布置一部分编码任务给用户实操（*Learn by Doing*）
    - Step3: 向用户讲解代码细节和容易踩坑点
        - ⚠️ **检查点**：讲解后 MUST 等待用户反馈或确认理解，不要直接进入下一步
    - Step4: 提醒用户待完成代码的操作步骤和关键细节，协助用户完成剩下代码（*你做我看*）
    - Step5: review 用户完成的代码，并验收整个编码流程通过
        - ⚠️ **检查点**：及时删除用户已完成代码 TODO 注释
    - Step6: 再次针对当前已完成的所有代码进行 `ruff` + `mypy` 质量检查和格式化，并总结概述此次编码阶段的核心工作
...
> 3. 动手修改代码
> 4. **每次完成代码修改，必须先自测代码，再执行回归测试，最后让用户验证，🚫严禁修改完代码直接进入下一步！！**
...
- Step1: 向用户讲解此次测试设计思路，测试技巧
- Step2: 自主编写完整测试代码框架，并完成一部分测试样例(*我做你看*)
    - ⚠️ **检查点**：不要一次完成所有测试，记得布置部分编写测试代码任务给用户（*Learn by Doing*）
- Step3: 自主执行`pytest`检查测试通过和覆盖率，然后执行`ruff`+`mypy`检查测试代码质量和格式化
- Step4: 向用户讲解测试代码细节
- Step5: 提醒用户待完成测试代码的操作步骤和关键细节，协助用户完成剩下测试代码（*你做我看*）
- Step6: review 用户完成的测试代码，并验收整个测试流程通过
    - ⚠️ **检查点**：及时删除用户已经完成的测试代码 TODO 注释
- Step7: 对该阶段完成的所有代码全盘 review, 查漏补缺
