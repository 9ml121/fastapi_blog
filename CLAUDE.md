...
-   `docs/design/phaseX_code_review_summary.md` - 代码 review 经验总结
...
-   ✅ MUST：如果用户只是询问问题，你在思考过程中认为需要修改代码，不要自主采取行动，务必先跟用户确认！
-   ✅ MUST：每次最多编辑一个文件，如果一个任务需要编辑多个文件，务必先更新 todo list,一步一步来
-   ✅ MUST：每编辑完一个文件，务必要先跟用户确认，不要马上编辑下一个
-   ✅ MUST：超过 10 行代码编辑，必须在编辑完代码之后，向用户解释你的代码
-   ❌ NEVER：一次性编辑多个文件，中间未向用户确认

**质量保障**

-   ✅ MUST：编写/修改代码超过 5 行 → 立即执行 `uv run ruff check path/*.py --fix` + `uv run mypy path/*.py`
-   ✅ MUST：重构代码务必执行回归测试确保功能无损
