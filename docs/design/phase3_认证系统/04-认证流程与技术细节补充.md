...
### 2.4 验证码生成：`random` vs `secrets`

Python 提供了两个随机数模块，选择错误会导致严重安全漏洞。

#### 核心区别

| 特性 | `random` | `secrets` |
|------|----------|-----------|
| **随机性来源** | 伪随机数生成器 (PRNG) | OS 密码学安全随机源 |
| **可预测性** | ⚠️ 种子相同则序列相同 | ✅ 不可预测 |
| **底层实现** | Mersenne Twister 算法 | `/dev/urandom` (Linux) / `CryptGenRandom` (Windows) |
...
import random
random.seed(12345)
print(random.randint(100000, 999999))  # 每次运行都是同一个值！
```

`random` 使用 Mersenne Twister 算法，攻击者收集约 624 个输出后可**重建内部状态并预测后续值**。
...
> **记忆口诀**：涉及安全用 `secrets`，其他场景用 `random`。
